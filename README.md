# About xoxo (forked from [github.com/knq/xo](https://github.com/knq/xo))

`xoxo` is a command-line tool to generate [Go](https://golang.org/project/)
code based on a database schema or a custom query.

`xoxo` works by using database metadata and SQL introspection queries to discover
...
Currently, `xoxo` can generate types for tables, enums, stored procedures, and
...
```sh
$ go get -u github.com/xoxo-go/xoxo

# install with oracle support (see notes below)
$ go get -tags oracle -u github.com/xoxo-go/xoxo
```

**_NOTE:_** Go 1.6+ is needed for installing `xoxo` from source, as it makes use
of the trim template syntax in Go templates, which is not compatible with
previous versions of Go. However, code generated by `xoxo` should compile with Go
1.3+.

## Quickstart

The following is a quick overview of using `xoxo` on the command-line:
...
$ xoxo pgsql://user:pass@host/dbname -o models

# generate code for a Microsoft SQL schema using a custom template directory (see notes below)
$ mkdir -p mssqlmodels
$ xoxo mysql://user:pass@host/dbname -o mssqlmodels --template-path /path/to/custom/templates

# generate code for a custom SQL query for postgres
$ xoxo pgsql://user:pass@host/dbname -N -M -B -T AuthorResult -o models/ << ENDSQL
SELECT
...
The following are `xoxo`'s command-line arguments and options:
...
`xoxo` provides a set of generic "base" [templates](templates/) for each of the
...
custom templates, or modify the base templates available in the `xoxo` source
tree, and use those with `xoxo` by a passing a directory path via the `--template-path`
flag.

For non-trivial schemas, custom templates are the most practical, common, and
best way to use `xoxo` (see below quickstart and related example).

### Custom Template Quickstart

The following is a quick overview of copying the base templates contained in
the `xoxo` project's [`templates/`](templates) directory, editing to suit, and
using with `xoxo`:
...
# copy xoxo templates for postgres
$ cp "$GOPATH/src/github.com/xoxo-go/xoxo/templates/*" templates/
...
# use with xo
$ xoxo pgsql://user:pass@host/db -o models --template-path templates
```

See the Custom Template example below for more information on adapting the base
templates in the `xoxo` source tree for use within your own project.
...
//go:generate xoxo pgsql://user:pass@host/db -o models --template-path templates
ENDGO
...
Note that `xoxo` only needs the templates for your specific database. You can
...
`xoxo` templates are standard Go text templates. Please see the [documentation
...
templates are instances of `xoxo/internal/$TYPE` (see below table on `$TYPE`
...
which defines the template used by `xoxo` for PostgreSQL's foreign keys. This
template will be called once for every foreign key relationship that `xoxo` finds
...
templates are not duplicated, but are instead symlinks in the `xoxo` source tree.
...
The base `xoxo` templates are bin packed so that they are always available to the
built `xoxo` binary using [`go-bindata`](https://github.com/jteeuwen/go-bindata) (via
the [`tpl.sh`](tpl.sh) script) and need to be regenerated/included in any
changeset when submitting any template changes to the `xoxo` project.

If you would like to distribute your own binary version of `xoxo` with the
included templates, simply modify the templates in the `xoxo` source tree, run
...
with `xoxo`, and the resulting code generated by `xoxo`.

Additionally, please see the [pokedex example](examples/pokedex) for a
demonstration of running `xoxo` against a large schema. Please note that this
...
instead of having them managed by code generated by `xoxo`. As such, when you
need `xoxo` to ignore fields for a database schema, you can use the `--ignore-fields`
...
code generated with `xoxo` by passing the `--ignore-fields` option:

```sh
# ignore special fields
$ xoxo pgsql://user:pass@host/db -o models --ignore-fields created_at modified_at
```
...
To accomplish this with `xoxo`, we will need to create our own set of custom
templates, and then add a `GetMostRecent` lookup to the `$DBTYPE.type.go.tpl`
template.

First, we create a copy of the base `xoxo` templates:
...
We can then use the templates in conjunction with `xoxo` to generate our "model"
code:

```sh
$ xoxo pgsql://user:pass@localhost/dbname -o models --template-path templates/
```
...
Please note that the base `xoxo` templates do not import any SQL drivers. It is
left for the user of `xoxo`'s generated code to import the actual drivers. For
...
```sh
$ xoxo --escape-all 'mysql://user:pass@host/?parseTime=true&sql_mode=ansi' -o models
```
...
```sh
$ xoxo 'mysql://user:pass@host/dbname?parseTime=true' -o models
```
...
used by `xoxo` needs the Oracle `instantclient` libs to be installed/known by
...
$ sudo cp $GOPATH/src/github.com/xoxo-go/xoxo/contrib/oci8.pc /usr/lib/pkgconfig/
...
$ go install -tags oracle github.com/xoxo-go/xoxo
```
...
For reference, the `xoxo` developers use the [sath89/oracle-12c](https://hub.docker.com/r/sath89/oracle-12c/) Docker image
for testing `xoxo`'s Oracle database support.

### SQLite3 (sqlite3)

While not required, one should specify the `loc=auto` option when using `xoxo`
with a SQLite3 database:

```sh
$ xoxo 'file:mydatabase.sqlite3?loc=auto' -o models
```
...
## About Primary Keys
For row inserts `xoxo` determines whether the primary key is
...
`xoxo` will query the schema to determine if the database provides an automatic primary key
...
`xoxo` can likely get you 99% "of the way there" on medium or large database
...
`xoxo` is **NOT** designed to be an ORM or to generate an ORM. Instead, `xoxo` is
...
`xoxo` was originally developed while migrating a large application written in
...
`xoxo` is the open source version of that code generation tool, and is the fruits
of those development efforts. It is hoped that others will be able to use and
expand `xoxo` to support other databases -- SQL or otherwise -- and that `xoxo` can
become a common tool in any Go developer's toolbox.

### Goals

Part of `xoxo`'s goals is to avoid writing an ORM, or an ORM-like in Go, and to
instead generate static, type-safe, fast, and idiomatic Go code across
languages and databases. Additionally, the `xoxo` developers are of the opinion
...
ie, a "self-documenting" schema. `xoxo` is an end to that pursuit.
